...
@Component({
  selector: 'ispl-complaint-resolution-pi-add',
  templateUrl: 'complaint-resolution-pi-add.component.html',
  styleUrls: ['complaint-resolution-pi-add.component.css']

})
export class ComplaintResoluionPIAddComponent {
  private plantType: string;
  public title: string = "Root Cause Analysis Modify";
  ...
  public rootCauseRemarksDetails: string = "";//textarea
  public validInvalidComplaintValue: string;//radio button value
  public remarksDetails: string = "";//text area value for remarks details

  //to store the maxlength for localstorage
  public actionRecomendedAfterSiteVisitLength: number = this.localStorageService.dbSettings.actionRecomendedAfterSiteVisit;
  public validInvalidComplaintRemarksLength: number = this.localStorageService.dbSettings.validInvalidComplaintRemarks;
  ...
  ngOnInit(): void {
    ...
    console.log("complaintReferenceNo for Root cause analysis add: ", this.complaintReferenceNo);
  ...
  //start method onValidInvalidComplaintRadioClick
  public onValidInvalidComplaintRadioClick(validInvalidValue) {
    console.log("commercialSettlementRadioValue:: ", validInvalidValue);
    this.validInvalidComplaintValue = validInvalidValue;
  }//end of method onValidInvalidComplaintRadioClick
  ...
  public getComplaintReferenceDetails(complaintReferenceNo: string,fileActivityId: number) {
    this.complaintResolutionPIService.getComplaintReferenceDetailsView(complaintReferenceNo,fileActivityId)
      .subscribe(res => {
        ...
        if(res.msgType == "Info"){
          let actionRecomendedAfterSiteVisitRes: string = this.selectedComplaintReferenceDetails.actionRecomendedAfterSiteVisit;
          this.rootCauseRemarksDetails = actionRecomendedAfterSiteVisitRes.trim();
          this.complaintResolutionPIFormGroup.controls["rootCauseRemarks"].setValue(this.rootCauseRemarksDetails);
          let validInvalidComplaintValueRes: string = this.selectedComplaintReferenceDetails.validComplaint;
          this.validInvalidComplaintValue = validInvalidComplaintValueRes.substring(0,1);

          let validInvalidComplaintRemarksFromRes: string = this.selectedComplaintReferenceDetails.validInvalidComplaintRemarks;
          this.remarksDetails = validInvalidComplaintRemarksFromRes.trim();  
          if(this.validInvalidComplaintValue == "N"){
            this.complaintResolutionPIFormGroup.controls["remarks"].setValue(this.rootCauseRemarksDetails);
          }else{
            this.complaintResolutionPIFormGroup.controls["remarks"].setValue(" ");
          }       
  ...
  public onComplaintResolutionPISubmit() {
    console.log("form value of root cause PI add/modify submit : ", this.complaintResolutionPIFormGroup.value);
    let complaintResoSubmitDet: any = {};
    complaintResoSubmitDet.complaintReferenceNo = this.complaintReferenceNo;
    complaintResoSubmitDet.actionRecomendedAfterSiteVisit = this.complaintResolutionPIFormGroup.value.rootCauseRemarks;//actn det
    complaintResoSubmitDet.validComplaint = this.validInvalidComplaintValue;// radio value
    if(this.validInvalidComplaintValue == "N"){
      complaintResoSubmitDet.validInvalidComplaintRemarks = this.complaintResolutionPIFormGroup.value.remarks;//commercial settlement
    }else{
      complaintResoSubmitDet.validInvalidComplaintRemarks = " ";
    }
    ...
    this.updateBusySpinner();
      this.complaintResolutionPIService.capasubmitWithFileUpload(formDataObj).
        subscribe(res => {
          ...
          if(res.msgType == "Info"){
            // if(fileBoolean == true){
              this.router.navigate([ROUTE_PATHS.RouteComplaintResolutionPI]);//route to the previous page
            // }
          }else{
            this.resErrorType =  this.errorConst;
    ...
    public fileChange(event) {
      ...
      if (this.fileList.length > 0) {
        for (let i: number = 0; i < this.fileList.length; i++) {
          ...
          console.log("file.size on root pi::::",file.size);  
