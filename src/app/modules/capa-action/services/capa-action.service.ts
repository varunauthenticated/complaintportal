...
export class CAPAActionService {
    ...
    private successCallbackForFileUpload(res: Response) {
        console.error("success of capa di submit with file upload in service class==>", res);
        return res.json();
    }
    private errorCallBackForFileUpload(error: Response) {
        console.error("error of capa submit di with file upload in service class==>", error);
        return Observable.throw(error);
    }
